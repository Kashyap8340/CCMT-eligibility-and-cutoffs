...
const yOff = 0.005;
const zOff = 0.0005;
const backgroundColor = '#F4F4F0'; // Bauhaus off-white
...
function initParticle(i) {
  ...
  // Pick Bauhaus Hue: Red (0), Yellow (48), Blue (224)
  const bauhausHues = [0, 48, 224];
  hue = bauhausHues[Math.floor(Math.random() * bauhausHues.length)];
...
function drawParticle(x, y, theta, life, ttl, size, hue) {
  ...
  ctx.a.lineCap = 'round';
  ctx.a.lineWidth = 1.5;
  ctx.a.strokeStyle = `hsla(${hue}, 80%, 50%, ${fadeInOut(life, ttl)})`;
  ctx.a.beginPath();
...
function createCanvas() {
  container = document.getElementById('ambient-bg');
	canvas = {
